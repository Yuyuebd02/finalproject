...
object yjp10 {
  def main(args: Array[String]): Unit = {
    ...
    //导入数据
    val df1_user = sc2.read.option("header","true").option("inferSchema","true").csv("input/credit.csv")
    df1_user.createOrReplaceTempView("credit")
    val df2 = sc2.sql(
      """SELECT Age,sum(Credit_amount)
        |FROM credit
